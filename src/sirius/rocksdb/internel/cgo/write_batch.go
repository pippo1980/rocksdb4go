...
func (wb *WriteBatch) DeleteRangeCF(cf *ColumnFamilyHandle, start []byte, end []byte) {
	cStart := byteToChar(start)
	cEnd := byteToChar(end)
	C.rocksdb_writebatch_delete_range_cf(wb.c, cf.c, cStart, C.size_t(len(start)), cEnd, C.size_t(len(end)))
}
